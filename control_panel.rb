...
class ControlPanel
  ...
  def run
    ...
    loop do
      ...
      puts '3.Управление маршрутами' if stations.count > 1
      puts '0.Выход'
  ...
  def train_manage
    loop do
      ...
      if trains.any?
        ...
        puts '6.Управлять движением поезда' if routes.any?
  ...
  def create_train
    begin
      ...
      user_choise = gets.chomp.to_i
      case user_choise
      when 1 then trains << PassengerTrain.new(train_number, company_name)
      when 2 then trains << CargoTrain.new(train_number, company_name)
  ...
  def list_all_trains
    trains.each.with_index(1) do |train, index|
      puts "#{index}. #{train.number} тип: #{train.type} вагоны: #{train.wagons.size}"
    end
  end
  ...
  def delete_wagon
    ...
    train.wagons.each_with_index do |wagon, index|
      puts "#{index}.#{wagon.company_name} тип: #{wagon.type}"
    end
  ...
  def station_manage
    loop do
      ...
      if stations.any?
        ...
        puts '5.Добавить груз поезду' if trains.any?
      end
      puts '0.Назад'
      user_choise = gets.chomp.to_i
      case user_choise
  ...
  def list_station_trains(station)
    ...
    station.each_train_with_index(1) do |train, index|
      puts "#{index}. #{train.number} тип: #{train.type} вагонов: #{train.wagons.count}"
    end
  end
